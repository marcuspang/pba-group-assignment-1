use array_bytes::hex2bytes;
use std::{
    collections::{BTreeMap, HashMap},
    iter::zip,
    string::FromUtf8Error,
};

const CIPHER_TEXTS: [&str; 9] = [
    "160111433b00035f536110435a380402561240555c526e1c0e431300091e4f04451d1d490d1c49010d000a0a4510111100000d434202081f0755034f13031600030d0204040e",
    "",
    "",
    "0b07540c1d0d0b4800354f501d131309594150010011481a1b5f11090c0845124516121d0e0c411c030c45150a16541c0a0b0d43540c411b0956124f0609075513051816590026004c061c014502410d024506150545541c450110521a111758001d0607450d11091d00121d4f0541190b45491e02171a0d49020a534f",
    "",
    "",
    "011b0d131b060d4f5233451e161b001f59411c090a0548104f431f0b48115505111d17000e02000a1e430d0d0b04115e4f190017480c14074855040a071f4448001a050110001b014c1a07024e5014094d0a1c541052110e54074541100601014e101a5c",
    "0c06004316061b48002a4509065e45221654501c0a075f540c42190b165c",
];

const KEY_LENGTH: usize = "".len();
const DUMMY_CHAR: u8 = b'~';
const FOUND_KEY: &str = "Bitcoin: A purely peer-to-peer version of electronic cash would allow online payments to be sent directly from one party to another without going through a financial institution";

fn xor_bytes(x: &[u8], y: &[u8]) -> Vec<u8> {
    zip(x, y).map(|(&a, &b)| a ^ b).collect::<Vec<u8>>()
}

// notice that most alphabets XOR-ed with each other result in <= 26,
fn xor_alphabets() {
    let alphabets = String::from("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNQOPRSTUVWXYZ");
    let alphabets = alphabets.as_bytes();

    for i in 0..52 {
        for j in 0..52 {
            if i == j {
                continue;
            }
            println!(
                "XOR {} with {}, {}",
                alphabets.get(i).unwrap(),
                alphabets.get(j).unwrap(),
                alphabets.get(i).unwrap() ^ alphabets.get(j).unwrap()
            );
        }
    }
}

fn frequency_analysis() -> Result<(), ()> {
    let ciphertexts_bytes = CIPHER_TEXTS.map(|text| hex2bytes(text).unwrap());

    for i in 0..ciphertexts_bytes.len() {
        let mut filtered_strings: Vec<Vec<u8>> = vec![];
        for j in i + 1..ciphertexts_bytes.len() {
            let xor = xor_bytes(
                ciphertexts_bytes.get(i).unwrap(),
                ciphertexts_bytes.get(j).unwrap(),
            );
            let filtered = xor
                .iter()
                .map(|&char| {
                    if !char.is_ascii_alphanumeric() {
                        return DUMMY_CHAR;
                    }
                    return char;
                })
                .collect::<Vec<u8>>();
            let filtered_string = String::from_utf8(filtered.clone()).unwrap();

            let mut counts = HashMap::new();
            for &item in &filtered {
                *counts.entry(item).or_insert(0) += 1;
            }
            let mut sorted_counts = BTreeMap::new();
            for (char, count) in counts {
                sorted_counts
                    .entry(count)
                    .or_insert_with(Vec::new)
                    .push(char);
            }

            for (count, chars) in sorted_counts.iter().rev() {
                for &char in chars {
                    println!("Character {:?} occurs {} times", char as char, count);
                }
            }

            filtered_strings.push(filtered.clone());

            println!("filtered text {i}->{j} {filtered_string}");
        }
    }

    Ok(())
}

fn decrypt(ciphertext: &str, key: &str) -> String {
    let cipher = hex2bytes(ciphertext).unwrap();
    let xor = xor_bytes(cipher.as_slice(), key.as_bytes());
    String::from_utf8(xor).unwrap()
}

fn main() {
    // frequency_analysis();
    // xor_alphabets();
    for ciphertext in CIPHER_TEXTS {
        let decrypted_text = decrypt(ciphertext, FOUND_KEY);
        println!("{}", decrypted_text);
    }
}
